...
function val_correo() {
    var correo_validation = /[\w-\.]{2,}@([\w-]{2,}\.)*([\w-]{2,}\.)[\w-]{2,4}/;
    if ($.trim($("#correo").val()).length == 0) {
        error_correo = "Complete este campo*";
        $("#error_correo").text(error_correo);
    } else if (!correo_validation.test($("#correo").val())) {
        error_correo = "Correo invalido!";
        $("#error_correo").text(error_correo);
    } else {
        error_correo = "";
        $("#error_correo").text(error_correo);
...
function val_correoe() {
    var correoe_validation = /[\w-\.]{2,}@([\w-]{2,}\.)*([\w-]{2,}\.)[\w-]{2,4}/;
    if ($.trim($("#correoe").val()).length == 0) {
        error_correoe = "Complete este campo*";
        $("#error_correoe").text(error_correoe);
    } else if (!correoe_validation.test($("#correoe").val())) {
        error_correoe = "Correo invalido!";
        $("#error_correoe").text(error_correoe);
    } else {
        error_correoe = "";
        $("#error_correoe").text(error_correoe);
    }
    return error_correoe;
}
